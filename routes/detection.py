...
async def detect_video_auto(
    ...
    Order: Gemini (best, when quota) -> local action model (fast, 13 crime
    types incl. accidents) -> Qwen VLM (thorough, slow). The first model that
    finds an incident wins; if none do, the video is reported clear.
    """
    if not file.filename:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Filename is required"
    ...
    try:
        ...
        # 1) Gemini — best + fast when the API key has quota.
        try:
        ...
        # 2) Dedicated violence model — accurate fight/assault detection.
        if detected is None:
            try:
                r = await run_in_threadpool(detect_violence, video_path=Path(tmp_path))
                if r.get("violence_detected"):
                    detected = r
                elif clear_result is None:
                    clear_result = r
            except Exception as e:
                logger.info("auto: violence model skipped (%s)", str(e)[:120])

        # 3) Local action model — covers 13 crime types incl. accidents/robbery.
        if detected is None:
            try:
                r = await run_in_threadpool(
                    detect_actions,
                    ...
                    window_seconds=3,
                    stride_seconds=2,
                    # High threshold: the action model over-guesses weak
                    # "Fighting/Assault" (~65%) on normal clips. Real crimes
                    # score 95%+; the dedicated violence model handles fights.
                    threshold=0.85,
                )
                if r.get("violence_detected"):
                    detected = r
                ...
                logger.info("auto: action model skipped (%s)", str(e)[:120])

        # (Qwen VLM is available at /detection/video/qwen for thorough manual
        # analysis, but kept out of the fast auto path — on 8 GB VRAM it's slow.)
